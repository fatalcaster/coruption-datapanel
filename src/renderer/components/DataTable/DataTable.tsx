...
export const DataTable: React.FC<DataTableProps> = ({
  ...
  const [removingRow, setRemovingRow] = useState<number | undefined>(undefined);
          ...
          <tbody>
            <tr>
              {headers.map((value, index) => {
                return <td key={`${name}-header-${index}`}>{value}</td>;
              })}
            </tr>
          </tbody>
        </table>
              ...
              ? data.map((value: any, index: number) => {
                  return removingRow != index ? (
                    <tr
                      key={`${name}-row-${index}`}
                      onContextMenu={() => {
                        if (onDelete) setRemovalContext(index);
                      }}
                    >
                      {typeof value !== "string" ? (
                        ...
                        <td key={`${name}-row-${index}-data-${index}`}>
                          {value}
                        </td>
                      )}
                    </tr>
